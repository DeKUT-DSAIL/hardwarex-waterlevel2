...
HCSR04 sensor(D12, D11);// water level sensor 
static AnalogIn battery_voltage (A0); //battery_voltage
...
int main () {
  ...
  while (1) {
    ...
    length1 = (uint16_t)(distance);  
    bat_voltage = battery_voltage.read_u16();
    

    printf("\rLevel: %d\n, \rVBAT:%d\n ",length1, bat_voltage );

    std::vector<uint8_t> tx_data;
    tx_data.push_back((length1 >> 8) & 0xFF);
    tx_data.push_back(length1 & 0xFF);
    tx_data.push_back((bat_voltage >> 8) & 0xFF);
    tx_data.push_back(bat_voltage & 0xFF);
